<template>
  <div>
    ...
    <p v-if="songs.length===0" style="color:#909399;padding:40px 0;text-align:center">还没有播放记录</p>
    <div style="text-align:center;margin-top:20px" v-if="total > size">
      <el-pagination background layout="prev, pager, next"
        v-model:current-page="page" :page-size="size" :total="total" @current-change="loadHistory" />
    </div>
  </div>
</template>
...
async function loadHistory() {
  try {
    const res = await request.get('/history', { params: { page: page.value, size } })
