<template>
  <div class="page-wrap">
    <div class="page-hero"><h2 class="page-title">🕐 播放历史</h2><p class="page-desc">回顾你的音乐旅程</p></div>
    <div class="song-grid"><SongCard v-for="s in songs" :key="s.id" :song="s" @click="goSong" /></div>
    <div class="empty" v-if="!songs.length"><span class="empty-icon">🎧</span><p>还没有播放记录</p><router-link to="/discover" class="empty-link">去听点什么吧 →</router-link></div>
    <div class="pagination-wrap" v-if="total>size"><el-pagination background layout="prev,pager,next" v-model:current-page="page" :page-size="size" :total="total" @current-change="loadHistory"/></div>
  </div>
</template>
<script setup>
...
async function loadHistory(){try{const r=await request.get('/history',{params:{page:page.value,size}});songs.value=r.data.list||[];total.value=r.data.total||0}catch(e){}}
